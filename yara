...
wapus = {
    GetValue = function(self, section, flag)
        ...
        -- Fallback defaults for flags accessed before initialization (prevents arithmetic crashes)
        local numericFlags = {
...
-- creds: linoria
-- Object, Toggled, FadeSpeed, TransparencyCache
...
function watermark:create()
	...
	function temptable:Set(tab) -- idfk what to name this -- { "version", "fps", "ping", "time" }
		pcall(function()
			local textTable = {}
...
do
	...
	function gpm.AddToggle(Options, OptionParent)
		...
		-- ── Container wraps the toggle row + the optional cog sub-panel ──
		-- AutomaticSize.Y lets it grow when the sub-panel opens.
		...
		-- The actual toggle row (LayoutOrder = 0 → always first)
		...
		-- Return Container (not ToggleHolder) so the cog sub-panel
		-- can be appended inside the same container.
...
local function CreateCogSubPanel(Container, Options)
	...
	local SubPanel = CreateInstance("Frame", {
		...
		BorderSizePixel  = 1,
		BorderMode       = Enum.BorderMode.Inset,
		LayoutOrder      = 1,   -- always after the toggle row (LayoutOrder 0)
		Visible          = false,
		Parent           = Container,
		["Children"] = {
			CreateInstance("UIStroke", {}),
        ...
        replicationInterface.operateOnAllEntries(function(player, entry)
            ...
            if character and entry._isEnemy then
                ...
                if not visibleCheck or not raycast(localposition, target - localposition, physicsignore) then
                    ...
                    if screenPosition.Z > 0 and screenDistance < distance and (not deadfov or screenDistance >= deadfov) then
        ...
        elseif name == "newbullets" then
            ...
            if wapus:GetValue("Silent Aim", "Enabled") and (wapus:GetValue("Silent Aim", "Hit Chance") >= chanceOne) then
                ...
                if target then
                    ...
                    local targetVelocity = (movementCache.position[player] and movementCache.position[player][15]) and (movementCache.position[player][15] - movementCache.position[player][1]) / (movementCache.time[15] - movementCache.time[1]) or Vector3.zero
                    local velocity = complexTrajectory(bulletData.firepos, publicSettings.bulletAcceleration, target, weaponInterface.getActiveWeaponController():getActiveWeapon()._weaponData.bulletspeed, targetVelocity).Unit
...
--[[ synz has upval instability
    ...
    table.insert(connectionList, runService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function(deltaTime)
        ...
        if wapus:GetValue("Aim Bot", "Enabled") and aiming then
            ...
            if target and movementCache.position[entry._player] and movementCache.position[entry._player][15] then
                aimbotting = true
...
LPH_NO_VIRTUALIZE(function() -- Make UI
    ...
    local menu = Library:New({ Name = "Wapus" })
    ...
    local legit = menu:AddTab("Legit")
    local rage = menu:AddTab("Rage")
    local visuals = menu:AddTab("Visuals")
    local misc = menu:AddTab("Misc")

    local aimbot = legit:AddSection("Aim Bot", "Left")
    local fovsettings = legit:AddSection("FOV Settings", "Left")
    local silentaim = legit:AddSection("Silent Aim", "Right")
    local backtrack = legit:AddSection("Backtracking", "Left")
    local hitboxes = legit:AddSection("Hit Boxes", "Left")
    local gunmods = legit:AddSection("Gun Mods", "Right")

    local ragebot = rage:AddSection("Rage Bot", "Left")
    local knifebot = rage:AddSection("Knife Bot", "Left")
    local antiaim = rage:AddSection("Anti Aim", "Right")
    local fakelag = antiaim

    local enemyesp = visuals:AddSection("Enemy ESP", "Left")
    local chams = visuals:AddSection("Chams", "Right")
    local morechams = visuals:AddSection("More Chams", "Right")
    local worldvisuals = visuals:AddSection("World Visuals", "Right")
    local thirdperson = visuals:AddSection("Third Person", "Left")
    local customChar = visuals:AddSection("Custom Model", "Left")

    local movement = misc:AddSection("Movement", "Left")
    local sounds = misc:AddSection("Sounds", "Left")
    local tweaks = misc:AddSection("Tweaks", "Right")
    local antivotekick = misc:AddSection("Anti Votekick", "Right")
    local chatspam = misc:AddSection("Chat Spam", "Right")
    local hopper = misc:AddSection("Server Hopper", "Right")
    ...
    hitboxes:AddToggle({ Name = "Enabled", Default = false, Callback = getCallback("Hit Boxes%%Enabled"), SaveAs = "Hit Boxes%%Enabled" }):AddKeybind({ Name = "Key Bind" }):AddColorpicker({ Name = "Color", Default = Color3.new(0.1, 0.1, 1), Callback = getCallback("Hit Boxes%%Color"), SaveAs = "Hit Boxes%%Color" })
    hitboxes:AddDropdown({ Name = "Hit Part", Default = "Head", Values = {"Head", "Torso"}, Callback = getCallback("Hit Boxes%%Hit Part"), SaveAs = "Hit Boxes%%Hit Part" })
    hitboxes:AddSlider({ Name = "Size", Min = 1, Max = 20, Default = 20, Precision = 1, Suffix = " Studs", Callback = getCallback("Hit Boxes%%Size"), SaveAs = "Hit Boxes%%Size" })
    hitboxes:AddSlider({ Name = "Transparency", Min = 0, Max = 100, Default = 50, Precision = 1, Suffix = "%", Callback = getCallback("Hit Boxes%%Transparency"), SaveAs = "Hit Boxes%%Transparency" })
    hitboxes:AddDropdown({ Name = "Material", Default = "SmoothPlastic", Values = {"ForceField", "SmoothPlastic", "Glass"}, Callback = getCallback("Hit Boxes%%Material"), SaveAs = "Hit Boxes%%Material" })

    backtrack:AddToggle({ Name = "Enabled", Default = false, Callback = getCallback("Backtracking%%Enabled"), SaveAs = "Backtracking%%Enabled" }):AddKeybind({ Name = "Key Bind" }):AddColorpicker({ Name = "Character Color", Default = Color3.new(0.1, 0.1, 1), Callback = getCallback("Backtracking%%Characters Color"), SaveAs = "Backtracking%%Characters Color" })
    backtrack:AddSlider({ Name = "Refresh Rate", Min = 1, Max = 10, Default = 2, Precision = 1, Suffix = " Characters/Second", Callback = getCallback("Backtracking%%Refresh Rate"), SaveAs = "Backtracking%%Refresh Rate" })
    backtrack:AddSlider({ Name = "Character Duration", Min = 0.1, Max = 1, Default = 1, Precision = 0.1, Suffix = " Seconds", Callback = getCallback("Backtracking%%Character Duration"), SaveAs = "Backtracking%%Character Duration" })
    backtrack:AddSlider({ Name = "Character Transparency", Min = 0, Max = 100, Default = 50, Precision = 1, Suffix = "%", Callback = getCallback("Backtracking%%Character Transparency"), SaveAs = "Backtracking%%Character Transparency" })
    backtrack:AddDropdown({ Name = "Character Material", Default = "ForceField", Values = {"ForceField", "SmoothPlastic", "Glass"}, Callback = getCallback("Backtracking%%Character Material"), SaveAs = "Backtracking%%Character Material" })
    backtrack:AddToggle({ Name = "Clone Character", Default = true, Callback = getCallback("Backtracking%%Clone Character"), SaveAs = "Backtracking%%Clone Character" })
    ...
    -- Rage Elements
    ragebot:AddToggle({ Name = "Enabled", Default = false, Callback = getCallback("Rage Bot%%Enabled"), SaveAs = "Rage Bot%%Enabled" }):AddKeybind({ Name = "Key Bind" })
    ragebot:AddToggle({ Name = "Shoot Effects", Default = false, Callback = getCallback("Rage Bot%%Shoot Effects"), SaveAs = "Rage Bot%%Shoot Effects" })
    ragebot:AddToggle({ Name = "Fire Position Scanning", Default = false, Callback = getCallback("Rage Bot%%Fire Position Scanning"), SaveAs = "Rage Bot%%Fire Position Scanning" })
    ragebot:AddSlider({ Name = "Fire Position Offset", Min = 1, Max = 15.9, Default = 9, Precision = 0.1, Suffix = " Studs", Callback = getCallback("Rage Bot%%Fire Position Offset"), SaveAs = "Rage Bot%%Fire Position Offset" })
    ragebot:AddToggle({ Name = "Hit Position Scanning", Default = false, Callback = getCallback("Rage Bot%%Hit Position Scanning"), SaveAs = "Rage Bot%%Hit Position Scanning" })
    ragebot:AddSlider({ Name = "Hit Position Offset", Min = 1, Max = 10, Default = 6, Precision = 0.1, Suffix = " Studs", Callback = getCallback("Rage Bot%%Hit Position Offset"), SaveAs = "Rage Bot%%Hit Position Offset" })
    ragebot:AddToggle({ Name = "Only Shoot Target Status", Default = false, Callback = getCallback("Rage Bot%%Only Shoot Target Status"), SaveAs = "Rage Bot%%Only Shoot Target Status" }):AddKeybind({ Name = "Target Key Bind" })
    ragebot:AddToggle({ Name = "Whitelist Friendly Status", Default = true, Callback = getCallback("Rage Bot%%Whitelist Friendly Status"), SaveAs = "Rage Bot%%Whitelist Friendly Status" }):AddKeybind({ Name = "Friendly Key Bind" })

    knifebot:AddToggle({ Name = "Kill All (May Despawn)", Default = false, Callback = getCallback("Knife Bot%%Kill All (May Despawn)"), SaveAs = "Knife Bot%%Kill All (May Despawn)" }):AddKeybind({ Name = "Key Bind" })
    knifebot:AddToggle({ Name = "Only When Holding Knife", Default = false, Callback = getCallback("Knife Bot%%Only When Holding Knife"), SaveAs = "Knife Bot%%Only When Holding Knife" })
    knifebot:AddToggle({ Name = "Only Kill Target Status", Default = false, Callback = getCallback("Knife Bot%%Only Kill Target Status"), SaveAs = "Knife Bot%%Only Kill Target Status" }):AddKeybind({ Name = "Target Key Bind" })
    knifebot:AddToggle({ Name = "Whitelist Friendly Status", Default = true, Callback = getCallback("Knife Bot%%Whitelist Friendly Status"), SaveAs = "Knife Bot%%Whitelist Friendly Status" }):AddKeybind({ Name = "Friendly Key Bind" })

    antiaim:AddToggle({ Name = "Enabled (May Cause Despawning)", Default = false, Callback = getCallback("Anti Aim%%Enabled (May Cause Despawning)"), SaveAs = "Anti Aim%%Enabled (May Cause Despawning)" })
    antiaim:AddToggle({ Name = "Yaw", Default = false, Callback = getCallback("Anti Aim%%Yaw"), SaveAs = "Anti Aim%%Yaw" })
    antiaim:AddSlider({ Name = "Yaw Amount", Min = 0, Max = 360, Default = 180, Precision = 1, Suffix = " Degrees", Callback = getCallback("Anti Aim%%Yaw Amount"), SaveAs = "Anti Aim%%Yaw Amount" })
    antiaim:AddDropdown({ Name = "Yaw Mode", Default = "Relative", Values = {"Relative", "Absolute"}, Callback = getCallback("Anti Aim%%Yaw Mode"), SaveAs = "Anti Aim%%Yaw Mode" })
    antiaim:AddToggle({ Name = "Pitch", Default = false, Callback = getCallback("Anti Aim%%Pitch"), SaveAs = "Anti Aim%%Pitch" })
    antiaim:AddSlider({ Name = "Pitch Amount", Min = 0, Max = 180, Default = 0, Precision = 1, Suffix = " Degrees", Callback = getCallback("Anti Aim%%Pitch Amount"), SaveAs = "Anti Aim%%Pitch Amount" })
    antiaim:AddDropdown({ Name = "Pitch Mode", Default = "Relative", Values = {"Relative", "Absolute"}, Callback = getCallback("Anti Aim%%Pitch Mode"), SaveAs = "Anti Aim%%Pitch Mode" })
    antiaim:AddToggle({ Name = "Spin Bot", Default = false, Callback = getCallback("Anti Aim%%Spin Bot"), SaveAs = "Anti Aim%%Spin Bot" })
    antiaim:AddSlider({ Name = "Spin Speed", Min = 0, Max = 1800, Default = 180, Precision = 1, Suffix = " Degrees/Second", Callback = getCallback("Anti Aim%%Spin Speed"), SaveAs = "Anti Aim%%Spin Speed" })
    antiaim:AddDropdown({ Name = "Spin Direction", Default = "Right", Values = {"Left", "Right"}, Callback = getCallback("Anti Aim%%Spin Direction"), SaveAs = "Anti Aim%%Spin Direction" })
    antiaim:AddToggle({ Name = "Jitter", Default = false, Callback = getCallback("Anti Aim%%Jitter"), SaveAs = "Anti Aim%%Jitter" })
    antiaim:AddSlider({ Name = "Jitter Speed", Min = 0, Max = 12, Default = 6, Precision = 1, Suffix = " Shakes/Second", Callback = getCallback("Anti Aim%%Jitter Speed"), SaveAs = "Anti Aim%%Jitter Speed" })
    antiaim:AddToggle({ Name = "Force Stance", Default = false, Callback = getCallback("Anti Aim%%Force Stance"), SaveAs = "Anti Aim%%Force Stance" })
    antiaim:AddDropdown({ Name = "Set Stance", Default = "Prone", Values = {"Stand", "Crouch", "Prone"}, Callback = getCallback("Anti Aim%%Set Stance"), SaveAs = "Anti Aim%%Set Stance" })

    fakelag:AddToggle({ Name = "Fake Lag", Default = false, Callback = getCallback("Fake Lag%%Enabled"), SaveAs = "Fake Lag%%Enabled" }):AddKeybind({ Name = "Key Bind" })
    fakelag:AddToggle({ Name = "Randomize Position", Default = false, Callback = getCallback("Fake Lag%%Randomize Position"), SaveAs = "Fake Lag%%Randomize Position" })
    fakelag:AddSlider({ Name = "X-Axis Factor", Min = 0, Max = 8.9, Default = 0, Precision = 1, Suffix = " Studs", Callback = getCallback("Fake Lag%%X-Axis Factor"), SaveAs = "Fake Lag%%X-Axis Factor" })
    fakelag:AddSlider({ Name = "Z-Axis Factor", Min = 0, Max = 8.9, Default = 0, Precision = 1, Suffix = " Studs", Callback = getCallback("Fake Lag%%Z-Axis Factor"), SaveAs = "Fake Lag%%Z-Axis Factor" })
    fakelag:AddSlider({ Name = "Refresh Distance", Min = 0, Max = 8.9, Default = 5, Precision = 0.1, Suffix = " Studs", Callback = getCallback("Fake Lag%%Refresh Distance"), SaveAs = "Fake Lag%%Refresh Distance" })
    fakelag:AddSlider({ Name = "Refresh Rate", Min = 0, Max = 10, Default = 1, Precision = 1, Suffix = " Seconds", Callback = getCallback("Fake Lag%%Refresh Rate"), SaveAs = "Fake Lag%%Refresh Rate" })

    -- Visuals Elements
    enemyesp:AddToggle({ Name = "Enabled", Default = true, Callback = getCallback("Enemy ESP%%Enabled"), SaveAs = "Enemy ESP%%Enabled" })
    enemyesp:AddToggle({ Name = "Boxes", Default = false, Callback = getCallback("Enemy ESP%%Boxes"), SaveAs = "Enemy ESP%%Boxes" }):AddColorpicker({ Name = "Box Color", Default = Color3.fromRGB(0, 255, 255), Callback = getCallback("Enemy ESP%%Box Color"), SaveAs = "Enemy ESP%%Box Color" })
    enemyesp:AddSlider({ Name = "Box Opacity", Min = 1, Max = 100, Default = 100, Precision = 1, Suffix = "%", Callback = getCallback("Enemy ESP%%Box Opacity"), SaveAs = "Enemy ESP%%Box Opacity" })
    enemyesp:AddToggle({ Name = "Box Outlines", Default = false, Callback = getCallback("Enemy ESP%%Box Outlines"), SaveAs = "Enemy ESP%%Box Outlines" }):AddColorpicker({ Name = "Box Outline Color", Default = Color3.fromRGB(0, 0, 0), Callback = getCallback("Enemy ESP%%Box Outline Color"), SaveAs = "Enemy ESP%%Box Outline Color" })
    enemyesp:AddSlider({ Name = "Box Outline Opacity", Min = 1, Max = 100, Default = 100, Precision = 1, Suffix = "%", Callback = getCallback("Enemy ESP%%Box Outline Opacity"), SaveAs = "Enemy ESP%%Box Outline Opacity" })
    enemyesp:AddToggle({ Name = "Fill Boxes", Default = false, Callback = getCallback("Enemy ESP%%Fill Boxes"), SaveAs = "Enemy ESP%%Fill Boxes" }):AddColorpicker({ Name = "Box Inside Color", Default = Color3.fromRGB(0, 255, 255), Callback = getCallback("Enemy ESP%%Box Inside Color"), SaveAs = "Enemy ESP%%Box Inside Color" })
    enemyesp:AddSlider({ Name = "Box Inside Opacity", Min = 1, Max = 100, Default = 100, Precision = 1, Suffix = "%", Callback = getCallback("Enemy ESP%%Box Inside Opacity"), SaveAs = "Enemy ESP%%Box Inside Opacity" })
    enemyesp:AddToggle({ Name = "Health Bar", Default = false, Callback = getCallback("Enemy ESP%%Health Bar"), SaveAs = "Enemy ESP%%Health Bar" }):AddColorpicker({ Name = "Damage Color", Default = Color3.fromRGB(255, 0, 0), Callback = getCallback("Enemy ESP%%Damage Color"), SaveAs = "Enemy ESP%%Damage Color" }):AddColorpicker({ Name = "Health Color", Default = Color3.fromRGB(0, 255, 0), Callback = getCallback("Enemy ESP%%Health Color"), SaveAs = "Enemy ESP%%Health Color" })
    enemyesp:AddToggle({ Name = "Health Bar Outline", Default = false, Callback = getCallback("Enemy ESP%%Health Bar Outline"), SaveAs = "Enemy ESP%%Health Bar Outline" }):AddColorpicker({ Name = "Health Outline Color", Default = Color3.fromRGB(0, 0, 0), Callback = getCallback("Enemy ESP%%Health Outline Color"), SaveAs = "Enemy ESP%%Health Outline Color" })
    enemyesp:AddToggle({ Name = "Tracers", Default = false, Callback = getCallback("Enemy ESP%%Tracers"), SaveAs = "Enemy ESP%%Tracers" }):AddColorpicker({ Name = "Tracer Color", Default = Color3.fromRGB(0, 255, 255), Callback = getCallback("Enemy ESP%%Tracer Color"), SaveAs = "Enemy ESP%%Tracer Color" })
    enemyesp:AddSlider({ Name = "Tracer Opacity", Min = 1, Max = 100, Default = 100, Precision = 1, Suffix = "%", Callback = getCallback("Enemy ESP%%Tracer Opacity"), SaveAs = "Enemy ESP%%Tracer Opacity" })
    enemyesp:AddToggle({ Name = "Tracer Outlines", Default = false, Callback = getCallback("Enemy ESP%%Tracer Outlines"), SaveAs = "Enemy ESP%%Tracer Outlines" }):AddColorpicker({ Name = "Tracer Outline Color", Default = Color3.fromRGB(0, 0, 0), Callback = getCallback("Enemy ESP%%Tracer Outline Color"), SaveAs = "Enemy ESP%%Tracer Outline Color" })
    enemyesp:AddSlider({ Name = "Tracer Outlines Opacity", Min = 1, Max = 100, Default = 100, Precision = 1, Suffix = "%", Callback = getCallback("Enemy ESP%%Tracer Outlines Opacity"), SaveAs = "Enemy ESP%%Tracer Outlines Opacity" })
    enemyesp:AddDropdown({ Name = "Tracer Origin", Default = "Bottom", Values = {"Middle", "Top", "Bottom"}, Callback = getCallback("Enemy ESP%%Tracer Origin"), SaveAs = "Enemy ESP%%Tracer Origin" })
    enemyesp:AddToggle({ Name = "Names", Default = false, Callback = getCallback("Enemy ESP%%Names"), SaveAs = "Enemy ESP%%Names" }):AddColorpicker({ Name = "Names Color", Default = Color3.fromRGB(255, 255, 255), Callback = getCallback("Enemy ESP%%Names Color"), SaveAs = "Enemy ESP%%Names Color" })
    enemyesp:AddToggle({ Name = "Weapons", Default = false, Callback = getCallback("Enemy ESP%%Weapons"), SaveAs = "Enemy ESP%%Weapons" }):AddColorpicker({ Name = "Weapons Color", Default = Color3.fromRGB(255, 255, 255), Callback = getCallback("Enemy ESP%%Weapons Color"), SaveAs = "Enemy ESP%%Weapons Color" })
    enemyesp:AddToggle({ Name = "Distances", Default = false, Callback = getCallback("Enemy ESP%%Distances"), SaveAs = "Enemy ESP%%Distances" }):AddColorpicker({ Name = "Distances Color", Default = Color3.fromRGB(255, 255, 255), Callback = getCallback("Enemy ESP%%Distances Color"), SaveAs = "Enemy ESP%%Distances Color" })
    enemyesp:AddToggle({ Name = "Health Percents", Default = false, Callback = getCallback("Enemy ESP%%Health Percents"), SaveAs = "Enemy ESP%%Health Percents" }):AddColorpicker({ Name = "Health Number Color", Default = Color3.fromRGB(255, 255, 255), Callback = getCallback("Enemy ESP%%Health Number Color"), SaveAs = "Enemy ESP%%Health Number Color" })
    enemyesp:AddToggle({ Name = "Text Outlines", Default = true, Callback = getCallback("Enemy ESP%%Text Outlines"), SaveAs = "Enemy ESP%%Text Outlines" }):AddColorpicker({ Name = "Text Outline Color", Default = Color3.fromRGB(0, 0, 0), Callback = getCallback("Enemy ESP%%Text Outline Color"), SaveAs = "Enemy ESP%%Text Outline Color" })
    enemyesp:AddToggle({ Name = "Highlight Chams", Default = false, Callback = getCallback("Enemy ESP%%Highlight Chams"), SaveAs = "Enemy ESP%%Highlight Chams" }):AddColorpicker({ Name = "Highlight Outline Color", Default = Color3.fromRGB(0, 0, 0), Callback = getCallback("Enemy ESP%%Highlight Outline Color"), SaveAs = "Enemy ESP%%Highlight Outline Color" }):AddColorpicker({ Name = "Highlight Fill Color", Default = Color3.fromRGB(0, 0, 255), Callback = getCallback("Enemy ESP%%Highlight Fill Color"), SaveAs = "Enemy ESP%%Highlight Fill Color" })
    enemyesp:AddSlider({ Name = "Highlight Fill Transparency", Min = 0, Max = 100, Default = 50, Precision = 1, Suffix = "%", Callback = getCallback("Enemy ESP%%Highlight Fill Opacity"), SaveAs = "Enemy ESP%%Highlight Fill Opacity" })
    enemyesp:AddSlider({ Name = "Highlight Outline Transparency", Min = 0, Max = 100, Default = 0, Precision = 1, Suffix = "%", Callback = getCallback("Enemy ESP%%Highlight Outline Opacity"), SaveAs = "Enemy ESP%%Highlight Outline Opacity" })
    enemyesp:AddToggle({ Name = "Highlight Visible Check", Default = false, Callback = getCallback("Enemy ESP%%Highlight Visible Check"), SaveAs = "Enemy ESP%%Highlight Visible Check" })

    chams:AddToggle({ Name = "Arm Chams", Default = false, Callback = getCallback("Chams%%Arm Chams"), SaveAs = "Chams%%Arm Chams" }):AddColorpicker({ Name = "Arm Color", Default = Color3.new(0.1, 0.1, 1), Callback = getCallback("Chams%%Arm Color"), SaveAs = "Chams%%Arm Color" })
    chams:AddSlider({ Name = "Arm Transparency", Min = 0, Max = 100, Default = 50, Precision = 1, Suffix = "%", Callback = getCallback("Chams%%Arm Transparency"), SaveAs = "Chams%%Arm Transparency" })
    chams:AddDropdown({ Name = "Arm Material", Default = "ForceField", Values = {"ForceField", "SmoothPlastic", "Glass"}, Callback = getCallback("Chams%%Arm Material"), SaveAs = "Chams%%Arm Material" })
    chams:AddToggle({ Name = "Gun Chams", Default = false, Callback = getCallback("Chams%%Gun Chams"), SaveAs = "Chams%%Gun Chams" }):AddColorpicker({ Name = "Gun Color", Default = Color3.new(0.1, 0.1, 1), Callback = getCallback("Chams%%Gun Color"), SaveAs = "Chams%%Gun Color" })
    chams:AddSlider({ Name = "Gun Transparency", Min = 0, Max = 100, Default = 50, Precision = 1, Suffix = "%", Callback = getCallback("Chams%%Gun Transparency"), SaveAs = "Chams%%Gun Transparency" })
    chams:AddDropdown({ Name = "Gun Material", Default = "ForceField", Values = {"ForceField", "SmoothPlastic", "Glass"}, Callback = getCallback("Chams%%Gun Material"), SaveAs = "Chams%%Gun Material" })

    morechams:AddToggle({ Name = "Third Person Character Chams", Default = false, Callback = getCallback("More Chams%%Third Person Character Chams"), SaveAs = "More Chams%%Third Person Character Chams" }):AddColorpicker({ Name = "Character Color", Default = Color3.new(0.1, 0.1, 1), Callback = getCallback("More Chams%%Character Color"), SaveAs = "More Chams%%Character Color" })
    morechams:AddSlider({ Name = "Character Transparency", Min = 0, Max = 100, Default = 50, Precision = 1, Suffix = "%", Callback = getCallback("More Chams%%Character Transparency"), SaveAs = "More Chams%%Character Transparency" })
    morechams:AddDropdown({ Name = "Character Material", Default = "ForceField", Values = {"ForceField", "SmoothPlastic", "Glass"}, Callback = getCallback("More Chams%%Character Material"), SaveAs = "More Chams%%Character Material" })
    ...
    thirdperson:AddToggle({ Name = "Camera Offset Always Visible", Default = true, Callback = getCallback("Third Person%%Camera Offset Always Visible"), SaveAs = "Third Person%%Camera Offset Always Visible" })
    thirdperson:AddToggle({ Name = "Apply Anti Aim To Character", Default = true, Callback = getCallback("Third Person%%Apply Anti Aim To Character"), SaveAs = "Third Person%%Apply Anti Aim To Character" })

    customChar:AddToggle({ Name = "Custom Character Model", Default = false, Callback = getCallback("Custom Model%%Custom Character Model"), SaveAs = "Custom Model%%Custom Character Model" }):AddKeybind({ Name = "Key Bind" })
    customChar:AddTextbox({ Name = "Asset ID", Default = "ID", Callback = getCallback("Custom Model%%Asset ID"), SaveAs = "Custom Model%%Asset ID" })
    customChar:AddSlider({ Name = "Asset Offset X", Min = -10, Max = 10, Default = 0, Precision = 0.2, Suffix = " Studs", Callback = getCallback("Custom Model%%Asset Offset X"), SaveAs = "Custom Model%%Asset Offset X" })
    customChar:AddSlider({ Name = "Asset Offset Y", Min = -10, Max = 10, Default = 0, Precision = 0.2, Suffix = " Studs", Callback = getCallback("Custom Model%%Asset Offset Y"), SaveAs = "Custom Model%%Asset Offset Y" })
    customChar:AddSlider({ Name = "Asset Offset Z", Min = -10, Max = 10, Default = 0, Precision = 0.2, Suffix = " Studs", Callback = getCallback("Custom Model%%Asset Offset Z"), SaveAs = "Custom Model%%Asset Offset Z" })
    ...
    movement:AddToggle({ Name = "Jump Power", Default = false, Callback = getCallback("Movement%%Jump Power"), SaveAs = "Movement%%Jump Power" }):AddKeybind({ Name = "Jump Bind" })
    movement:AddSlider({ Name = "Height Addition", Min = 1, Max = 15, Default = 10, Precision = 1, Suffix = " Studs", Callback = getCallback("Movement%%Height Addition"), SaveAs = "Movement%%Height Addition" })
    ...
    movement:AddToggle({ Name = "Bunny Hop", Default = false, Callback = getCallback("Movement%%Bunny Hop"), SaveAs = "Movement%%Bunny Hop" }):AddKeybind({ Name = "BHop Bind" })
    movement:AddToggle({ Name = "Only While Jumping", Default = true, Callback = getCallback("Movement%%Only While Jumping"), SaveAs = "Movement%%Only While Jumping" })
    ...
    tweaks:AddToggle({ Name = "Custom Kill Notification", Default = false, Callback = getCallback("Tweaks%%Custom Kill Notification"), SaveAs = "Tweaks%%Custom Kill Notification" })
    tweaks:AddTextbox({ Name = "Notification Text", Default = "Furry Killed!", Callback = getCallback("Tweaks%%Notification Text"), SaveAs = "Tweaks%%Notification Text" })
    tweaks:AddButton({ Name = "Unlock All Attachments", Callback = getCallback("Tweaks%%Unlock All Attachments") })
    tweaks:AddButton({ Name = "Unlock All Knives", Callback = getCallback("Tweaks%%Unlock All Knives") })
    tweaks:AddButton({ Name = "Unlock All Camos", Callback = getCallback("Tweaks%%Unlock All Camos") })
    tweaks:AddButton({ Name = "Unlock All", Callback = getCallback("Tweaks%%Unlock All") })

    antivotekick:AddButton({ Name = "Initiate Multi-Instance Anti Votekick", Callback = function() writefile(folderName .. "/cache/votekick data/" .. fileName, userName) end })
    antivotekick:AddButton({ Name = "Copy YouTube Tutorial Link", Callback = function() setclipboard("https://youtu.be/dvyiz8iVe5g") end })

    chatspam:AddToggle({ Name = "Enabled", Default = false, Callback = getCallback("Chat Spam%%Enabled"), SaveAs = "Chat Spam%%Enabled" }):AddKeybind({ Name = "Key Bind" })
    chatspam:AddDropdown({ Name = "Spam List", Default = "default.txt", Values = chatListsFiles, Callback = getCallback("Chat Spam%%Spam List"), SaveAs = "Chat Spam%%Spam List" })
    chatspam:AddSlider({ Name = "Spam Delay", Min = 2.51, Max = 5, Default = 2.51, Precision = 0.01, Suffix = " Seconds", Callback = getCallback("Chat Spam%%Spam Delay"), SaveAs = "Chat Spam%%Spam Delay" })

    hopper:AddButton({ Name = "Server Hop", Callback = getCallback("Server Hopper%%Server Hop") })
    hopper:AddButton({ Name = "Rejoin", Callback = getCallback("Server Hopper%%Rejoin") })
    hopper:AddButton({ Name = "Copy Join Script", Callback = getCallback("Server Hopper%%Copy Join Script") })
    hopper:AddButton({ Name = "Clear Cached Servers", Callback = getCallback("Server Hopper%%Clear Cached Servers") })
